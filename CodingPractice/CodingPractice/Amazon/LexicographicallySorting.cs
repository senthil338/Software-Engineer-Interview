  ...
  public  class LexicographicallySorting
    {
        ...
        /// <returns></returns>
        public static List<string> Sort(string[] boxList)
        {
            for (int i = 0; i < boxList.Length - 1; i++)
            {
                for (int j = i + 1; j > 0; j--)
                {
                    string left = boxList[j - 1].Substring(boxList[j - 1].IndexOf(' ') + 1);
                    string right = boxList[j].Substring(boxList[j].IndexOf(' ') + 1);
                    ...
                    if (string.Compare(left.Replace(" ", ""), right.Replace(" ", "")) == 0)
                    {
                        if (string.Compare(boxList[j - 1].Split(' ')[0], boxList[j].Split(' ')[0]) > 0)
                        {
                            Swap(boxList, j);
                        }
                    }
        ...
        private static void Swap(string[] inputArray, int j)
        {
